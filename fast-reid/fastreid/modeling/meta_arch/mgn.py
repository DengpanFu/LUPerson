...
class MGN(nn.Module):
    def __init__(self, cfg):
        ...
        extra_bn   = cfg.MODEL.BACKBONE.EXTRA_BN
        # fmt :on
        ...
        self.has_extra_bn = extra_bn
        if extra_bn:
            self.heads_extra_bn_b1 = get_norm(bn_norm, 2048)
            self.heads_extra_bn_b2 = get_norm(bn_norm, 2048)
            self.heads_extra_bn_b3 = get_norm(bn_norm, 2048)
    ...
    def forward(self, batched_inputs):
        ...
        if self.has_extra_bn: b1_feat = self.heads_extra_bn_b1(b1_feat)

        # branch2
        b2_feat = self.b2(features)
        if self.has_extra_bn: b2_feat = self.heads_extra_bn_b2(b2_feat)
        b21_feat, b22_feat = torch.chunk(b2_feat, 2, dim=2)

        # branch3
        b3_feat = self.b3(features)
        if self.has_extra_bn: b3_feat = self.heads_extra_bn_b3(b3_feat)
