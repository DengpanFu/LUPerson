...
def build_optimizer(cfg, model):
    ...
    for key, value in model.named_parameters():
        ...
        if "heads" in key: # for projection head, they may have larger LR
            lr *= cfg.SOLVER.HEADS_LR_FACTOR
        else: # for backbone, they may have different for convs and bns
            if 'bn' in key or 'downsample.1' in key:
                lr *= cfg.SOLVER.BACKBONE_BN_LR_FACTOR
            elif 'backbone.1' in key and isinstance(model.backbone[1], nn.BatchNorm2d):
                lr *= cfg.SOLVER.BACKBONE_BN_LR_FACTOR
        if "bias" in key:
            lr *= cfg.SOLVER.BIAS_LR_FACTOR
            weight_decay = cfg.SOLVER.WEIGHT_DECAY_BIAS
        params += [{"name": key, "params": [value], "lr": lr, 
                    "weight_decay": weight_decay, "freeze": False}]
